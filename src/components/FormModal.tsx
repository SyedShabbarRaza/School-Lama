...
const SubjectForm = dynamic(() => import("./forms/SubjectForm"), {
  loading: () => <h1 className="h-full w-full flex items-center justify-center">Loading...</h1>,
});
...
const FormModal = ({
  ...
  const deleteFn = deleteActionMap[table as keyof typeof deleteActionMap];
  const [state, formAction] = useFormState(
    deleteFn ?? (async () => ({ success: false, error: true })),
...
if (type === "delete" && id) {
  return (
    ...
    return <p className="text-center text-red-500">Form not found for {table}</p>;
  };
